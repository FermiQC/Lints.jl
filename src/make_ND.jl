function make_2D(destination,engine,basis)
    ...
    for _i=1:s, _j=_i:s
        ...
        chonk = Lints.chunk(S_engine,i,j,basis) .- 1
        _chonk = chonk .+ 1
...
function make_4D(destination,engine,basis)
    s = getsize(basis)
    donesies = []
    for _i=1:s,_j=1:s,_k=1:s,_l=1:s
        ...
        sp = Lints.startpoint(engine,i,j,k,l,basis) .+ 1
        chonk = Lints.chunk(engine,i,j,k,l,basis) .- 1
        _chonk = chonk .+ 1
        r1 = sp[1]:chonk[1]+sp[1]
        r2 = sp[2]:chonk[2]+sp[2]
        r3 = sp[3]:chonk[3]+sp[3]
        r4 = sp[4]:chonk[4]+sp[4]
        shell = permutedims(reshape(Lints.compute(engine,i,j,k,l,basis),Tuple(reverse(_chonk))),(4,3,2,1))
        I[r1,r2,r3,r4] .= shell
